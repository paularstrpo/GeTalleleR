...
if (numel(layer)==1){#plot single layer (raw data + fitted modes)
    ...
    for (i in 1:nb_of_mwindows){
        ...
        if (!isempty(s_data)){
            ...
            hold on
            s_window <- sort(win_idx,'asc}')
            idx_jump <- find(diff(s_window)>1)){
            nb_jumps <- numel(idx_jump)
            win_edges <- []
            if (nb_jumps>0){
                win_edges_idx <- [s_window(1) s_window(idx_jump) s_window(idx_jump+1)  s_window(length(s_window))]
                win_edges_val <- data.segment(sort(win_edges_idx),1)
                n_edges <- 3*(1+nb_jumps)
                win_edges(1:3:n_edges) <- win_edges_val(1:2:length(win_edges))
                win_edges(2:3:n_edges) <- win_edges_val(2:2:length(win_edges))
                win_edges(3:3:n_edges) <- NaN
            } else {
                win_edges <- data.segment([s_window(1) s_window(length(s_window))],1)
            }
    ...
    plot([data.segment(1,1) data.segment(},1)],[0.5 0.5],'k:')
