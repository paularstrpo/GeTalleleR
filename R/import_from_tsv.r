import_from_tsv <- function(filename,purity){
...
for (i in 1:4){
    ...
    if (strcmp(lr_str,'Ne')){
        lyr(1) <- i
    } else if (strcmp(lr_str,'Nt')){
        lyr(2) <- i
    } else if (strcmp(lr_str,'Te')){
        lyr(3) <- i
    } else if (strcmp(lr_str,'Tt')){
        lyr(4) <- i
    } else {
        disp('Warning')
    }
...
#chromosome
cells4 <- from_file.textdata(2:4:},1)
chromosomes in cellfun(@double,cellfun(@string,cells4,'UniformOutput',0))){){
chromosomes(isnan(chromosomes)) in cellfun(@double,cellfun(@char,cells4(isnan(chromosomes)),'UniformOutput',0))){){
# 88 is X; 89 is Y; 77 is M

if (mod(nb_rows,4)!=0){
    pos <- cellfun(@(x) str2double(x),from_file.textdata(2:},2))
    for (i in 1:4:numel(pos)){
